...
Mở `OUTPUT/function-d/05_test_case_spec.md`, chọn **một** test case, đi ngược đủ 4 chặng:
...
Đổi tên tạm `knowledge/function-d.md` → `_off.md`, chạy lại B1+B2 với `task-slug = function-d-noknow`.
...
| B2 hỏi lại đúng câu BA đã trả lời | Agent không đọc `knowledge/function-d.md` | Kiểm dòng "Đọc trước" có file knowledge chưa |
